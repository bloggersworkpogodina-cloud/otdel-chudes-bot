...
def rules_keyboard() -> InlineKeyboardMarkup:
    if WEBAPP_URL:
        # В групповых чатах web_app-кнопка может давать BUTTON_TYPE_INVALID.
        # Поэтому используем обычную URL-кнопку на красивую страницу правил.
        url = WEBAPP_URL if WEBAPP_URL.endswith("/rules") else f"{WEBAPP_URL}/rules"
        button = InlineKeyboardButton(text="📜 Правила чата", url=url)
    else:
